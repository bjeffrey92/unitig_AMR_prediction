...
def load_data(Ab: str, left_out_cluster: int) -> tuple:
    ...
    validation_labels = (
        Series(train_labels)[validation_indices_1].tolist()
        + Series(test_labels)[validation_indices_2].tolist()
    )

    kernel_dir = f"kernel_learning/{Ab}/gwas_filtered/cluster_CV"
    k_train_file = os.path.join(
        kernel_dir, f"clade_{left_out_cluster}_left_out_k_train.pkl"
    )
    k_test_file = os.path.join(
        kernel_dir, f"clade_{left_out_cluster}_left_out_k_test.pkl"
    )
    k_validate_file = os.path.join(
        kernel_dir, f"clade_{left_out_cluster}_left_out_k_validate.pkl"
    )
...
def train_evaluate(Ab, left_out_cluster, C, epsilon, verbose=False):
    ...
    train_acc = mean_acc_per_bin(
        torch.as_tensor(model.predict(k_train)),
        torch.as_tensor(training_labels),
    )
    test_acc = mean_acc_per_bin(
        torch.as_tensor(model.predict(k_test)), torch.as_tensor(testing_labels)
    )
...
if __name__ == "__main__":
    ...
    for left_out_cluster in list(range(1, 13)):
        ...
        out_dir = (
            f"kernel_learning/{Ab}/cluster_wise_cross_validation/SVR_results"
        )
