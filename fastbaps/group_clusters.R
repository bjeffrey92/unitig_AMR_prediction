...
compact.clusters <- function(tree, df, min.clusters = 10,
    ...
    while (n.clusters > min.clusters) {
        print(sprintf(
            "Smallest Cluster Size = %s, %s clusters in total",
            min(cluster.sizes$Freq),
            n.clusters
        ))

        smallest.cluster <- subset(
            cluster.sizes,
            Freq == min(cluster.sizes$Freq)
        )$Var1

        # finds cluster most likely to be the neighbour of smallest cluster and adds smallest cluster nodes to this
        neighbouring.cluster <- get.neighbouring.cluster(
            tree,
            smallest.cluster, df
        )
...
plot.clusters <- function(tree, plot.df) {
    gg <- ggtree(tree)
    f2 <- facet_plot(gg,
        panel = "fastbaps", data = plot.df, geom = geom_tile,
