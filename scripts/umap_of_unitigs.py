...
def load_data(data_dir):
    ...
    train_labels = torch.load(
        os.path.join(data_dir, "training_labels.pt")
    ).tolist()
    ...
    test_labels = torch.load(
        os.path.join(data_dir, "testing_labels.pt")
    ).tolist()
...
def main(Ab):
    ...
    embedding_df.to_csv(
        f"umap_results/jaccard_distance/{Ab}_umap_embedding.csv"
    )

    plt.clf()
    plt.scatter(
        x=embedding_df[0], y=embedding_df[1], c=embedding_df.r_s_labels
    )
    plt.savefig(
        f"umap_results/jaccard_distance/{Ab}_umap_coloured_by_resistance_status.png"
    )
