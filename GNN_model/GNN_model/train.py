...
def batch_train(
    ...
    if testing_data:
        loss_test, acc_test = test(
            testing_data, model, loss_function, accuracy, adj
        )
    else:
        loss_test = "N/A"
        acc_test = "N/A"
...
def train(
    ...
    if validation_data:
        loss_val, acc_val = test(
            validation_data, model, loss_function, accuracy, adj
        )
    else:
        loss_val = "N/A"
        acc_val = "N/A"
...
def load_data(
    ...
    training_metadata, testing_metadata = utils.load_metadata(
        data_dir
    )  # for CV split

    if left_out_clade is not None:
        training_indices = training_metadata.loc[
            training_metadata.Clade != left_out_clade
        ].index
        testing_indices = testing_metadata.loc[
            testing_metadata.Clade != left_out_clade
        ].index
    ...
    if left_out_cluster is not None:
        training_indices = training_metadata.loc[
            training_metadata.clusters != left_out_cluster
        ].index
        testing_indices = testing_metadata.loc[
            testing_metadata.clusters != left_out_cluster
        ].index
        validation_indices_1 = training_metadata.loc[
            training_metadata.clusters == left_out_cluster
        ].index  # extract data from training set
    ...
    training_data = utils.DataGenerator(
        training_features, training_labels, global_node=False
    )
    testing_data = utils.DataGenerator(
        testing_features, testing_labels, global_node=False
    )
    validation_data = utils.DataGenerator(
        validation_features, validation_labels, global_node=False
    )
...
def main(args):
    ...
    for left_out_clade in [1, 2, 3]:
        ...
        all_data = load_data(
            data_dir, distances=False, adj=True, left_out_clade=left_out_clade
        )
        ...
        torch.save(
            model, f"{Ab}_clade_{left_out_clade}_left_out_GCNPerNode.pt"
        )
