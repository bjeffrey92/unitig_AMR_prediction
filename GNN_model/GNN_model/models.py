...
from GNN_model.layers import (
    GraphConvolution,
    GraphConvolutionPerNode,
    PreSelectionConvolution,
    SparsePrePrunedLayer,
)
...
class GraphConnectionsNN(nn.Module):
    def __init__(self, distances, n_hid_1, n_hid_2, out_dim, dropout):
        ...
        self.layer_1 = SparsePrePrunedLayer(distances)
...
class GCNGlobalNode(nn.Module):
    ...
    def forward(self, x, adj):
        x = F.leaky_relu(self.gc1(x, adj))[
            -1
        ]  # select last node which is the global node
...
class GCNPerNode(nn.Module):
    def __init__(self, n_feat, n_hid_1, n_hid_2, out_dim, dropout, n_hid_3=0):
